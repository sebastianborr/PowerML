...
def pretty_params_from_row(row: pd.Series, arch_name: str) -> str:
    ...
    if pd.notna(params_str):
        ...
        if ps and ps.lower() != "nan":
            items = [p.strip() for p in re.split(r",\s*", ps) if p.strip()]
            if has_model:
                parts.append(latex_escape(model_name))
                parts.extend(latex_escape(it) for it in items)
                return ":\\newline " + ",\\newline ".join(parts[1:])
            else:
                return ",\\newline ".join(latex_escape(it) for it in items)
