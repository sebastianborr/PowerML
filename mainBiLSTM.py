...
from tensorflow.keras.layers import (LSTM, Conv1D, MaxPooling1D, Dropout, Dense, 
                                     Input, Bidirectional, Multiply, Concatenate,Flatten)
...
epoch_rate = 50  # Número de épocas
batch_size_rate = 64  # Tamaño del lote
NUM_FEATURES = X_exog.shape[1]-2  # Número de características de entrada

# Definir el modelo Bi-LSTM con atención (paper 38)
# Mecanismo de atención para características (Feature Attention)
...
def temporal_attention(inputs):
    # inputs shape: (batch_size, seq_length, units)
    seq_length = inputs.shape[1]
    units = inputs.shape[-1]
    
    # Calcular scores de atención (por paso temporal)
    attention = Dense(1, activation='relu')(inputs)  # Shape: (batch_size, seq_length, 1)
    attention = tf.keras.layers.Flatten()(attention)  # Shape: (batch_size, seq_length)
    attention = Dense(seq_length, activation='softmax')(attention)  # Shape: (batch_size, seq_length)
    attention = tf.keras.layers.Reshape((seq_length, 1))(attention)  # Shape: (batch_size, seq_length, 1)
    
    # Multiplicar los pesos de atención por la salida de la Bi-LSTM
    return Multiply()([inputs, attention])  # Broadcast: (48, 128) * (48, 1) -> (48, 128)

# Arquitectura completa
def build_bi_lstm_attention_model(seq_length, num_features):
    inputs = Input(shape=(seq_length, num_features))
    
    # Capa Bi-LSTM
    bi_lstm = Bidirectional(LSTM(128, return_sequences=True))(inputs)
    bi_lstm = Dropout(0.2)(bi_lstm)
    
    # Atención a características
    feature_att = feature_attention(bi_lstm)
    
    # Segunda capa Bi-LSTM + Atención temporal
    bi_lstm2 = Bidirectional(LSTM(64, return_sequences=True))(feature_att)
    temporal_att = temporal_attention(bi_lstm2)
    
    # Capas densas
    flattened = Flatten()(temporal_att)
    dense1 = Dense(1024, activation='relu')(flattened)
    output = Dense(1)(dense1)  # Predicción puntual
    
    return Model(inputs, output)

model = build_bi_lstm_attention_model(SEQ_LENGTH, NUM_FEATURES)
...
if 1:
    ...
    if 1:
        ...
        results_df.to_csv('./output/ResultsDL/01_BiALSTM_Tetuan_Paper.csv', index=False)